...
bool RH_RF69::init()
{
    if (!RHSPIDriver::init()) //setup spi
	return false; //if spi fails, return.
...
void RH_RF69::setOpMode(uint8_t mode)
{
   /*  uint8_t opmode = spiRead(RH_RF69_REG_01_OPMODE);
    ...
    spiWrite(RH_RF69_REG_01_OPMODE, mode);
...
{
    if (_mode != RHModeIdle)  //continue only if mode is not already idle.
    {
	/* if (_power >= 18)
	{
...
{
    //for now just making this as simple as possible...
    spiWrite(RH_RF69_REG_2E_SYNCCONFIG, 145); //auto restart, sync on, fill auto, sync size 2 bytes

    spiWrite(RH_RF69_REG_2F_SYNCVALUE1, 0x5A);
    spiWrite(RH_RF69_REG_2F_SYNCVALUE2, 0x5A);
